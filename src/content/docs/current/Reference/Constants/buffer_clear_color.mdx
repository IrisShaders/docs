...
This directive is used when clearing is enabled, and it defines the value stored in a [colortex](/current/reference/buffers/colortex/) or [shadowcolor](/current/reference/buffers/shadowcolor/) buffer after the clear operation (see [buffer clear](/current/reference/constants/buffer_clear/)).

Replace `<bufferName>` with the name of a sampler (e.g. `colortex2` or `shadowcolor0`).

By default, all [colortex](/current/reference/buffers/colortex/) buffers are cleared to 0s (black), except `colortex0`, which is cleared to the current fog color, and `colortex1`, which is cleared to white. All [shadowcolor](/current/reference/buffers/shadowcolor/) buffers are cleared with white by default. This directive allows changing this clear color for each attachment individually.
