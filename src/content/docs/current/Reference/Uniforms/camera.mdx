...
## vehicleLookVector <Badge text="Iris Exclusive" variant="tip" size="medium" />
```glsl
...
## relativeVehiclePosition <Badge text="Iris Exclusive" variant="tip" size="medium" />
```glsl
uniform vec3 relativeVehiclePosition;
```
This value stores the offset from [`cameraPosition`](/current/reference/uniforms/camera/#cameraposition) to the position of the currently used vehicle.
