...
The chunk offset in model space for the currently rendering chunk of terrain. Used in combination with [`vaPosition`](/current/reference/attributes/gl_vertex#in-vec3-vaposition) to get the model space position as follows:
...
:::caution[Warning]
In Minecraft 1.21.2, this attribute was renamed to `modelOffset` in the vanilla game. Iris has not reflected this change, so you should still use `chunkOffset`.
:::
...
:::caution[Warning]
This variable may be patched to a non-uniform value despite the `uniform` storage qualifier.
:::
