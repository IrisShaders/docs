...
```
This value is `true` when a vehicle currently used by the player is in water, and `false` otherwise.
...
```
This value is `true` when the player's feet are in water and `false` otherwise.
