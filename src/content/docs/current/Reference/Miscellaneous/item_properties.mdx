...
The `item.properties` file lets you assign IDs to specific items and armor by their Minecraft ID. The file should be placed in the `shaders` folder. The ID can be read in two different ways for different contexts:
...
```properties title=item.properties
