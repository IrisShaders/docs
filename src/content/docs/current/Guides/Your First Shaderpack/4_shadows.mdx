...
:::note
Full code for this stage of the tutorial is available for reference [on GitHub](https://github.com/IrisShaders/tutorial-code/tree/main/Your%20First%20Shader/4.%20Shadows).
:::
...
:::caution[Warning]
Preprocessor directives do _not_ have a semicolon on the end.
:::
...
To apply this distortion when sampling the shadow map, we will `#include /lib/distort.glsl` in `composite.fsh`, and then apply the distortion to our shadow clip position _after_ the bias.
...
While we're here, it's a good idea to set another value - [`shadowDistanceRenderMul`](/current/reference/constants/shadowdistancerendermul/). This tells Iris not to render terrain outside the shadow distance when rendering the shadow map. This functionality is disabled by default for legacy reasons, but there's no reason not to set it.
...
First, we should define how widely we want to sample, and how we want to distribute samples within that area. We can do this with preprocessor directives which we can later make configurable in the settings menu.
